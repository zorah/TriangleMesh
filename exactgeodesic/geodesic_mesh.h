...
inline void Mesh::build_adjacencies()
{
	...
	for(unsigned i=0; i<half_edges.size();)
	{
		...
		if(!(e.length() > 1e-100)){		//algorithm works well with non-degenerate meshes only
            std::string errormsg = "Mesh is degenerated at (" + to_string(e.adjacent_vertices()[0]->id()) + ", " + to_string(e.adjacent_vertices()[1]->id()) +  ").";
	...
	for(unsigned i=0; i<m_faces.size(); ++i)
	{
		...
		for(unsigned j=0; j<3; ++j)		//compute angle adjacent to the vertex j
		{
			...
			if(!(angle>1e-5)){
                throw geodesic_error("Mesh is degenerated, angle too small at face " + std::to_string(i) + ".");
